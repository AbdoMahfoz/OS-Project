...
void * create_page_table(uint32 *ptr_page_directory,
	...
	uint32* va = kmalloc(PAGE_SIZE);
	uint32 pa = kheap_physical_address((unsigned int) va);
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(pa,
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	for (int i = 0; i < e->page_WS_max_size; i++) {
		...
		if (addr >= virtual_address
				&& addr < virtual_address + (PAGE_SIZE * size)) {
	...
	for (int i = 0; i < size; i++) {
		...
		get_page_table(e->env_page_directory,
				(void*) (virtual_address + (i * PAGE_SIZE)), &pg_table);
		if (pg_table != NULL) {
			bool eraseIt = 1;
