...
void remove_pages(struct Env * e, uint32 PTBR, uint32 size) {
	while (PTBR--) {
		uint32 MinTime = 0xFFFFFFFF, j = 0;
		int uk;
		for (uk = 0; uk < size; uk++) {
			uint32 cTime = e->ptr_pageWorkingSet[uk].time_stamp;
			if (cTime < MinTime && !(e->ptr_pageWorkingSet[uk].empty)) {
				j = uk;
				MinTime = cTime;
			}
		}
		uint32 *ptr_page_table, *cva =
				(uint32*) e->ptr_pageWorkingSet[j].virtual_address;
		struct Frame_Info *PFI;
		PFI = get_frame_info(e->env_page_directory, cva, &ptr_page_table);
		if (ptr_page_table[PTX(cva)] & PERM_MODIFIED)
			pf_update_env_page(e, cva, PFI);
		unmap_frame(e->env_page_directory, cva);
...
void FreeWs(struct Env * curenv, bool x) {
	...
	if (x || size == curenv->page_WS_max_size) {
		...
		while (c < NumberOfPagesToBeRemoved) {
			...
			uint32 min = 0;
			while (1) {
				if (env_page_ws_is_entry_empty(curenv, index) == 1) {
					index++;
				} else {
					min = env_page_ws_get_time_stamp(curenv, index);
					break;
				}
			}
			...
			for (int q = index; q < curenv->page_WS_max_size; q++) {
				if (env_page_ws_is_entry_empty(curenv, q) == 1) {
					...
					uint32 temp = env_page_ws_get_time_stamp(curenv, q);
					if (min > temp) {
						min = temp;
						index = q;
					}
...
void ClearWs(struct Env * curenv) {
	...
	int c = 0;
	while (c <= size) {
		if (size == 0 || index>=curenv->page_WS_max_size)
			return;
		while (1) {
			if (env_page_ws_is_entry_empty(curenv, index) == 1) {
				if (index < curenv->page_WS_max_size){
					cprintf("empty Page \n");
					index++;
				}				else if (index >= curenv->page_WS_max_size){
					return;
				}
				break;
			} else {
