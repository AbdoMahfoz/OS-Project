...
void _main(void)
{
	...
	//[3] Test Re-allocation
	{
		...
		if ((sys_calculate_free_frames() - freeFrames) != 256 + 1) panic("Wrong free of the re-allocated space: WS pages in memory and/or page tables are not freed correctly");
		...
		if((sys_calculate_free_frames() - freeFrames) != 768 + 2) panic("Wrong re-allocation: either extra pages are re-allocated in memory or pages not allocated correctly on PageFile");
